...
- v21 배포 코드의 번호 너비 계산에 `번`을 포함하여 본문 첫 줄 들여쓰기도 함께 조정합니다.
...
원본 ZIP의 `src/`, `shared/`와 실제 v21 배포 번들에는 기존부터 차이가 있습니다. 이번 변경은 양쪽에 적용했으며, 기존 v21의 본문·표·조건 배치를 보존했습니다. 배포용 v21 코드는 `assets/index-v21.js`입니다. 수정 후 `npm run build:v21`로 압축 코드와 Worker용 사본을 갱신할 수 있습니다. 전체 검사: `npm test`.
