...
const YEAR_OPTIONS=Array.from({length:16},(_,i)=>String(CURRENT_YEAR+5-i));
...
const DEFAULT_TYPES={
  ...
  "생명과학":["특성","탐구","생명활동과 에너지1","생명활동과 에너지2","신경","막전위","근수축","호르몬과 항상성1","호르몬과 항상성2","병원체","방어","염색체/세포주기","핵형분석/세포매칭","세포매칭","사람의 유전","가계도","돌연변이","생태계1","생태계2","생태계3"]
};
...
function questionYear(q){const parsed=q?.createdAt?new Date(q.createdAt).getFullYear():CURRENT_YEAR;return String(q?.year||(!Number.isNaN(parsed)?parsed:CURRENT_YEAR));}
function normalizeQuestion(q){return{...q,year:questionYear(q)};}
function loadState(){try{const v=JSON.parse(localStorage.getItem(STORE_KEY));if(v&&Array.isArray(v.questions))return {...v,questions:v.questions.map(normalizeQuestion),seasons:v.seasons?.length?v.seasons:defaultSeasons(),types:v.types||DEFAULT_TYPES};}catch(e){}return{questions:[],seasons:defaultSeasons(),types:DEFAULT_TYPES,updatedAt:null,dirty:false};}
...
function refreshYearSelect(value=CURRENT_YEAR){options($("#year"),YEAR_OPTIONS,String(value));}
...
async function saveQuestion(event){event.preventDefault();const id=$("#questionId").value||uid(),existing=state.questions.find(q=>q.id===id);const q={id,subject:$("#subject").value,year:$("#year").value,season:$("#season").value,round:+$("#round").value,number:+$("#questionNo").value,type:$("#type").value,difficulty:$("#difficulty").value,score:+$("#score").value,source:$("#source").value.trim(),answer:$("#answer").value.trim(),explanationHtml:$("#explanation").innerHTML.trim(),explanationText:$("#explanation").innerText.trim(),memo:$("#memo").value.trim(),imageFileId:existing?.imageFileId||"",imageFileName:existing?.imageFileName||"",imageUrl:existing?.imageUrl||"",createdAt:existing?.createdAt||now(),updatedAt:now()};const duplicate=state.questions.find(x=>x.id!==id&&x.subject===q.subject&&questionYear(x)===q.year&&x.season===q.season&&+x.round===q.round&&+x.number===q.number);if(duplicate&&!confirm(`${q.year}년 ${q.subject} ${q.season} ${q.round}회 ${q.number}번이 이미 있습니다. 계속 저장할까요?`))return;if(imageDraft)await dbSet(id,imageDraft);const idx=state.questions.findIndex(x=>x.id===id);if(idx>=0)state.questions[idx]=q;else state.questions.unshift(q);persist(true);$("#questionId").value=id;$("#deleteBtn").classList.remove("hidden");renderArchive();renderAnalysis();showToast("브라우저에 저장했습니다.");if(config().url&&config().key)await syncToServer("push",true);}
...
=======
async function verifySettings(){const btn=$("#saveSettingsBtn"),url=$("#scriptUrl").value.trim(),key=$("#syncKey").value.trim();if (
  !/^https:\/\/script\.google\.com\/macros\/s\/[^/]+\/exec$/.test(url) ||
  !key
) {
  return showToast("/exec로 끝나는 웹 앱 배포 URL과 동기화 키를 확인하세요.");
...
>>>>>>> c22099bdaf8cb9a1f7805226e72316afd4423b2c
