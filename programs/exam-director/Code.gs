...
function doGet(e) {
  try {
    ...
    if (action === 'ping') return json_({ ok: true, message: 'connected', version: 1 });
...
function backupNow_(label) {
  ...
  var json=JSON.stringify({label:label,questions:readQuestions_(),seasons:readSeasons_(),types:readTypes_()});
