...
for(const sourceFile of sourceFiles){
  ...
  const details=lines.map(line=>{
    ...
    return{id:row.id,grade:row.grade,subject:row.subject,year:row.year,month:row.month,exam:row.exam,number:row.number,type:canonicalType(row.subject,typeMap[row.id]||row.type),prompt:row.prompt,view:row.view,choices:[row.c1,row.c2,row.c3,row.c4,row.c5],answer:row.answer,explanation:row.explanation,imageName:row.imageName,imageUrl:row.imageUrl,problemUrl:row.problemUrl,solutionUrl:row.solutionUrl,status:row.status};
  });
  details.forEach((row,rowIndex)=>searchIndex.push({i:row.id,g:row.grade,s:row.subject,y:Number(row.year)||0,m:Number(row.month)||0,e:row.exam,n:row.number,t:row.type||"",q:String(row.prompt||"").replace(/\s+/g," ").trim().slice(0,260),st:row.status,im:row.imageUrl||"",p:row.problemUrl||"",sol:row.solutionUrl||"",d:shardName,r:rowIndex,x:normalize([row.id,row.grade,row.subject,row.exam,row.number,`${row.number}번`,`${String(row.number).padStart(2,"0")}번`,row.type,row.prompt,row.view,...row.choices,row.answer,row.explanation].filter(Boolean).join(" "))}));
